...
async def test_default_default_headers():
    ...
    async with run_server(config):
        async with httpx.AsyncClient() as client:
            ...
            assert response.headers["server"] == "quanshu" and response.headers["date"]
    ...
    async with run_server(config):
        async with httpx.AsyncClient() as client:
            ...
            assert (
                ...
                and response.headers["server"] == "quanshu"
                and response.headers["date"]
            )


@pytest.mark.skip(reason="disable date header not supported yet.")
@pytest.mark.asyncio
